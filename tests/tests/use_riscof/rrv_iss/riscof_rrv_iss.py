...
class rrv_iss(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","rrv-iss")
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for rrv_iss start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' --rv-arch-test-signature={1} --rv-arch-test-signature-granularity=4 -f {2}'.format(self.isa, sig_file, elf)
          else:
            simcmd = 'echo "NO RUN"'

          # concatenate all commands that need to be executed within a make-target.
          execute = '@cd {0}; {1}; {2};'.format(testentry['work_dir'], cmd, simcmd)
          logger.debug('Executing on RRV-ISS {0}'.format(execute))
