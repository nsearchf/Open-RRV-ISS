...
setup() {
    echo -e "\n========= Running $0 starting ========="
    cd ./tests/tests/data/FreeRTOS-LTS-local
    make build_for_riscv --no-print-directory  --silent
    cd - > /dev/null
...
setup
output=$(./target/debug/rrv-iss -f ./tests/tests/data/FreeRTOS-LTS-local/build/rv32_example --steps 3000000)
echo "$output"
teardown
